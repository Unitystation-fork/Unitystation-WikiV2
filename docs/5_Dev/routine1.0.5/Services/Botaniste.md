```properties
Botanic
    Basique
        Accéder à son labo
        Accédder à son casier
        Récupérer son outillage + sac de jardinage
        Récupérer les graines dans le distributeur ou via le cargo
            planter
            arroser
            fertlisant
            retirer les mauvaises herbes
            récolter la plante
            A maturité
        Plante stock dans le sac jardinage
        - Par le seau + high capacity water tank : permet d'arroser une plante
        Enregistre la plante dans le seed extractor
        Retirer des graines du seed extractor
        Lacher/Peut donner des graines aux poulets
    Avancée
        High tower cap (champ) + une hache + le menu craft/clic directement le champ : découpe pour obtenir des planches
        Wooden barrel : faire fermenter des plantes 
        Drying rack :  ... ?
        Loom : produire du tissu
        plant dna manipulator : (minigame) ... ?
    Spécial
        se mettre un sceau sur la tête
        lancer de hache

Loom
    consome 4 "raw of cotton bundle" produit 1 "stack of cloth"
    consome 4 "raw of durathread bundle", produit 1 "stack of durathread"

Wooden barrel
    ouvrir/fermer
    ajouté des plante
    moonflower  = absinte
    harebell    = vermouth
    tabacco     = creme de menthe
    poppy       = vermouth

Poulet
    Doit recherche des graines au sol
    Manger des seeds / blé
    se reproduit lorsque 2 poules présentes + nourriture

Sac de jardinage
    Absober toute vegeteaux si en main
    vidable entièrement au sol
    + propriété d'ouverture d'un sac commun

Hydroponic
    dysfonctionnel => ingénieurie
    mature => plante prete
    orange => manque de fertilsant
    rouge => présence de mauvaise herbe
    bleu => manque d'eau

    Si Plante présent + mettre nouvelle graine
    Alors plante présenté devient fertilisant + nouvelle plante

Seed Extractor
    Analyse la plante insérer + consommation de la plante
    imprimer des graines (utilisable par le joueur) - limite un sac de graine par analyse
    
Seeds list

	|====== tubercule ================			
	| pack of cabbage seeds					
	| pack of garlic seeds						
	| pack of onion seeds						
	| pack of red onion seeds					
	| pack of red-beet seeds					
	| pack of white-beet seeds					
	| pack of carrot seeds						
	| pack of parsnip seeds					
	| pack of potato seeds						
	| pack of sweet potato seeds	Potato		
	| pack of blumpkin seeds					
	| pack of pumpkin seeds					
	| pack of holymelon seeds					
	| pack of watermelon seeds					
	|
	====== Bush =====================			
	| pack of berry seeds						
	| pack of death-berry seeds				
	| pack of glow-berry seeds				
	| pack of poison-berry seeds				
	| Toechtauese-Berries seeds		xxx		
	| pack of chili seeds						
	| pack of chilly pepper seeds				
	| pack of ghost chili seeds				
	| pack of Bell Pepper seeds	xxx			
	| pack of blood-tomato seeds				
	| pack of bluespace tomato seeds			
	| pack of blue-tomato seeds				
	| pack of killer-tomato seeds				
	| pack of tomato seeds					
	| pack of grape seeds						
	| pack of green grape seeds				
	| pack of eggplant seeds					
	
	====== Tree =====================			
	| pack of apple seeds						
	| pack of golden apple seeds				
	| pack of banana seeds					
	| pack of bluespace banana seeds			
	| pack of mimana seeds					
	| pack of blue cherry pits				
	| pack of cherry pits						
	| pack of cherryBulb pits					
	| pack of orange seeds					
	| pack of extradimensional orange seeds	
	| pack of lemon seeds						
	| pack of lime seeds						
	| pack of KortaNuts seeds			xxx		
	| pack of SweetKortaNuts seeds	xxx		
	
	===== Special ============================
	| pack of gatfruit seeds					
	| pack of NoFruit seeds		xx			
	| pack of replica pod seeds				
	| pack of combustible seeds				
	| pack of corpse flower seeds				
	| pack of eggy-plant seeds				
	
	=================================	
	| _SeedsLavandBase		
	| pack of aloe seeds			Exist		
	| pack of bungo seeds						
	| pack of galaxythistle seeds				
	| pack of fruiting cactus seeds-- Polypore
	| pack of death-nettle seeds Exist		
	| pack of nettle seeds	Exist			
	| pack of pineapple seeds					
	| pack of koibean seeds					
	| pack of soybean seeds					
	
	====== FLOWER Grass ================	
	| pack of spaceman's trumpet seeds		
	| pack of starthistle seeds-Noharvest		
	| pack of geranium seeds					
	| pack of harebell seeds					
	| pack of lily seeds						
	| pack of sunflower seeds					
	| pack of moonflower seeds				
	| pack of novaflower seeds				
	| pack of poppy seeds						
	| pack of rainbow bunch seeds				
	| pack of carpet seeds	-- Grass		
	| pack of fairygrass seeds				
	| pack of grass seeds						
	| pack of kudzu seeds						
	| pack of herb seeds				xxx		
	
	====== Cereal ====================	
	| pack of bamboo seeds					
	| pack of sugarcane seeds					
	| pack of vanilla seeds					
	| pack of cotton seeds					
	| pack of durathread seeds				
	| pack of wheat seeds						
	| pack of Meatwheat seeds					
	| pack of oat seeds						
	| pack of rice seeds						
	| pack of corn seeds						
	| pack of snapcorn seeds					
	
	====== Drogu ====================	
	| pack of cocoa seeds						
	| pack of coffee arabica seeds			
	| pack of coffee robusta seeds			
	| pack of tea aspera seeds				
	| pack of tea astra seeds NoHarvest		
	| pack of ambrosia deus seeds				
	| pack of ambrosia gaia seeds				
	| pack of ambrosia vulgaris seeds			
	| pack of space tobacco seeds				
	| pack of tobacco seeds					
	| pack of cannabis seeds					
	| pack of death cannabis seeds			
	| pack of life cannabis seeds				
	| pack of omega cannabis seeds			
	| pack of rainbow cannabis seeds			
	
	======= shroom ================	
	| pack of embershroom mycelium			
	| pack of chanterelle mycelium			
	| pack of destroying angel mycelium		
	| pack of fly amanita mycelium			
	| pack of glowcap mycelium				
	| pack of glowshroom mycelium				
	| pack of jupiter cup mycelium			
	| pack of liberty-cap mycelium			
	| pack of plump-helmet mycelium			
	| pack of reishi mycelium					
	| pack of shadowshroom mycelium			
	| pack of steel-cap mycelium				
	| pack of tower-cap mycelium				
	| pack of walking mushroom mycelium		
	| pack of inocype mycelium				
	| pack of porcini mycelium-PLANT EXIST	
	| pack of seraka cap mycelium		xx		
	| pack of Polypore mycelium  		xx	
	
```
